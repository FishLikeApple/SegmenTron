...
def load_model_pretrain(model, device):
    if cfg.PHASE == 'train':
        if cfg.TRAIN.PRETRAINED_MODEL_PATH:
            ...
            if device != None:
                state_dict_to_load = torch.load(cfg.TRAIN.PRETRAINED_MODEL_PATH, map_location=device)
            else:
        ...
        if cfg.TEST.TEST_MODEL_PATH:
            ...
            if device != None:
                state_dict_to_load = torch.load(cfg.TEST.TEST_MODEL_PATH, map_location=device)
            else:
                state_dict_to_load = torch.load(cfg.TEST.TEST_MODEL_PATH)
