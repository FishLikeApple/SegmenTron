    ...
    def train(self):
        ...
        for (images, targets, _) in self.train_loader:
            ...
            for i in range(image_arrays.shape[0]):
                ay = (np.swapaxes(image_arrays[i], 0, 2)).astype(np.uint8)
                print('ay: ', np.swapaxes(image_arrays[i], 0, 2))
                im = Image.fromarray(ay)
                im.save("./image{}.jpeg".format(i))
                _ay = np.expand_dims(target_arrays[i], 2)
                ay = (np.concatenate([_ay, _ay, _ay], 2)).astype(np.uint8)
                print('ay: ', np.concatenate([_ay, _ay, _ay], 2))
                im = Image.fromarray(ay)
